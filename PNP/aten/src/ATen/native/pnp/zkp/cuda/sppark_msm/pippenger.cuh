...
class msm_t {
 ...
 public:
  msm_t(
    ...
    static bool printed_ctor = false;
    if (!printed_ctor) {
        printed_ctor = true;
        printf("[MSM-CTOR] npoints=%zu wbits=%u nwins=%u row_sz=%u smcount=%zu\n",
              npoints, wbits, nwins, row_sz, smcount);
    }
    size_t d_buckets_sz =
