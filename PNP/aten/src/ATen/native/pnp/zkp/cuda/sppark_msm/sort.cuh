...
// // 只打印一次，避免刷屏：
// // 只在第一个 block、第一个线程、且只对一种 slice 打印
// if (blockIdx.x == 0 && threadIdx.x == 0) {
//     uint32_t buckets = 1u << bits;

//     uint32_t prev = 0;
//     uint32_t nonempty_bucket_count = 0;
//     uint32_t max_bucket_size = 0;
//     unsigned long long sum_bucket_size = 0;

//     for (uint32_t b = 0; b < buckets; ++b) {
//         uint32_t end = histogram[b];
//         uint32_t sz = end - prev;
//         prev = end;
...
//     double avg_bucket_size = buckets ? double(sum_bucket_size) / double(buckets) : 0.0;
//     double avg_nonempty_bucket_size =
//         nonempty_bucket_count ? double(sum_bucket_size) / double(nonempty_bucket_count) : 0.0;

//     printf("[LOWER-SORT-BUCKET] bits=%u buckets=%u nonempty=%u max=%u sum=%llu avg_all=%.3f avg_nonempty=%.3f\n",
//            bits,
//            buckets,
//            nonempty_bucket_count,
//            max_bucket_size,
