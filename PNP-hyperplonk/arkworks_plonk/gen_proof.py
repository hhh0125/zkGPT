...
def load_witnesses(witness_dir):
    w_l_scalar = torch.tensor(
        np.load(witness_dir+"/w_l_scalar"+"-12.npy", allow_pickle=True), dtype=fr.TYPE(), device="cuda"
    )
    w_r_scalar = torch.tensor(
        np.load(witness_dir+"/w_r_scalar"+"-12.npy", allow_pickle=True), dtype=fr.TYPE(), device="cuda"
    )
    w_o_scalar = torch.tensor(
        np.load(witness_dir+"/w_o_scalar"+"-12.npy", allow_pickle=True), dtype=fr.TYPE(), device="cuda"
    )
    w_4_scalar = torch.tensor(
        np.load(witness_dir+"/w_4_scalar"+"-12.npy", allow_pickle=True), dtype=fr.TYPE(), device="cuda"
    )
